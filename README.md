...
.
├── src/
│   ├── apps/          C entry points: robot simulator, gateway, and state dump tool
│   ├── core/          reusable C modules: MQTT, gateway, SQLite, WebSocket, telemetry cache
│   ├── sensors/       simulated sensor producers
│   └── generated/     generated protobuf-c source files
├── include/
│   ├── sensors/       public sensor headers
│   └── generated/     generated protobuf-c headers
├── proto/             protobuf schema
├── bin/               build output binaries; created by make and ignored by git
├── build/             object files and dependency files; created by make and ignored by git
├── ui/                browser supervisory interface
├── experiments/       automated runners, Playwright automation, and analysis scripts
├── exp/               recommended location for generated databases, UI logs, and results
├── paper/             optional publication-ready figures and tables copied by analysis
└── scripts/           utility scripts that are not part of the main experiment runner
```

The repository no longer keeps C sources, headers, object files, and executables in the root directory. Object files are produced under `build/`, and executable binaries are produced under `bin/`.
...
To print the build layout without compiling, run:

```bash
make layout
```

The generated protobuf-c files are already included. After editing `proto/robot_telemetry.proto`, regenerate them with:

```bash
make proto
```

If you use a ZIP archive created on another operating system and scripts are not executable, rebuild with `make clean && make` and run shell scripts using `bash <script>`.
...
```bash
./bin/robot_sim --host 127.0.0.1 --port 1883 --rate 100 --state-rate 100
```
