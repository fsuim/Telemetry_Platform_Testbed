...
# Strict automated disconnect+replay runner with gateway readiness checks.
# Run from repository root.
#
# Full replay experiment:
#   REPS=10 RATES="25 50 75 100 150" bash experiments/disconnect_replay_rate_sweep/run_disconnect_replay_matrix_auto.sh
#
# Single run:
#   HEADLESS=0 REPS=10 RATES="150" START_REP=7 END_REP=7 bash experiments/disconnect_replay_rate_sweep/run_disconnect_replay_matrix_auto.sh

SCRIPT_VERSION="v2_strict_wait_gateway_2026-06-23"
STUDY_ID=${STUDY_ID:-disconnect_replay_rate_sweep_auto_v2_strict_wait_gateway}
...
cat <<INFO
============================================================
Automated disconnect+replay telemetry-rate sweep — ${SCRIPT_VERSION}
Study ID: $STUDY_ID
Rates: $RATES
...
UI logs: $UI_LOG_DIR
Replay window: rate_hz * ${CUT_DURATION_S}s
Strict rule: each run must connect, reconnect, issue exactly one history request, and load the expected replay window.
============================================================
INFO

for rate in $RATES; do
  replay_samples=$(( rate * CUT_DURATION_S ))
  for rep in $(seq "$START_REP" "$END_REP"); do
    run_tag="disconnect_replay_r${rate}_rep${rep}"
    ...
    echo "Replay window: ${replay_samples} samples"
    ...
    ws_error_rows=$(awk -F, '$2=="ws_error" {c++} END {print c+0}' "$expected_log")
    ...
    history_request_last=$(awk -F, '$2=="history_request" {print $10; exit}' "$expected_log")
    history_loaded_count=$(awk -F, '$2=="history_batch_loaded" {print $11; exit}' "$expected_log")
...
PY
    ...
    echo "Validation: ws_open=${ws_open_rows}, ws_error=${ws_error_rows}, live_rows=${live_rows}, replay_rows=${replay_rows}, history_request=${history_requests}, history_loaded=${history_loaded}, request_last=${history_request_last:-NA}, loaded_count=${history_loaded_count:-NA}, expected_replay=${replay_samples}, min_live_rows=${min_live_rows}"
    ...
    if [[ "$history_requests" -ne 1 || "$history_loaded" -ne 1 ]]; then
      echo "ERROR: invalid disconnect+replay run ${run_tag}: expected exactly one history_request and one history_batch_loaded."
      exit 4
    fi
    if [[ "${history_request_last:-}" != "$replay_samples" ]]; then
      echo "ERROR: invalid disconnect+replay run ${run_tag}: history_request last=${history_request_last:-NA}, expected ${replay_samples}."
      exit 5
    fi
    if [[ "${history_loaded_count:-}" != "$replay_samples" ]]; then
      echo "ERROR: invalid disconnect+replay run ${run_tag}: history_batch_loaded count=${history_loaded_count:-NA}, expected ${replay_samples}."
      exit 6
    fi
    if [[ "$replay_rows" -ne "$replay_samples" ]]; then
      echo "ERROR: invalid disconnect+replay run ${run_tag}: replay rows=${replay_rows}, expected ${replay_samples}."
      exit 7
    fi
  done
done
...
echo
echo "Automated disconnect+replay runs complete. Suggested combined analysis:"
